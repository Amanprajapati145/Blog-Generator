...
import { signupSchema, signupUser } from "@/schema/user.schema";
...
const Signup = () => {
  const {
    ...
    formState: { errors },
    reset,
  } = useForm<signupUser>({ resolver: zodResolver(signupSchema) });

  const { register: signup, loading, error } = useUserStore();
